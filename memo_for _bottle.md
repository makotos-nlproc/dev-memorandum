...
BottleはPythonのWSGIのマイクロウェブフレームワークです。単一のファイルとして配布されPython標準ライブラリ以外の依存関係を持ちません。
...
route()デコレータはコードをURLパスに結びつける。この例の場合、/helloのパスにhello()関数を結びつけたということになる。

このrouteはbottleにおいてもっとも重要な概念だ。routeはしたいようにいくつも定義することができる。ブラウザがURLをリクエストしたとき、そのURLパスに関連づいた関数が呼び出され、その戻り値がブラウザに返される。

run()の呼び出しは組み込みの開発サーバを開始させる。ここではlocalhostの8080ポートで起動している。ctrl+cが打たれるまで、リクエストを受け続ける。このサーバはセットアップを一切必要とせず、アプリケーションを作り始め、ローカルテストで動かすのに驚くほど簡単案方法だ。

デバックモードは開発初期段階ではかなり便利なものだが、パブリックなアプリケーションではスイッチオフするのを忘れないよう心に留めておきましょう。

bottleでアプリケーションを作る方法の基本的な概念のデモはこれだけだ。続けて他に何ができるかを見ていこう。

### THE DEFAULT APPLICATION
簡単のためにこのチュートリアルではルートを定義するのに、ほとんどの例でモジュールレベルのroute()デコレータを使用する。これはルートをグローバルなdefault applicationに追加する。default applictionはroute()を初めて呼び出した際、自動的に生成されるBottleインスタンスである。他のモジュールレベルのデコレータや関数がこのdefault applictionに関連づいているが、もしオブジェクト指向的なアプローチを好み、余計なタイピングが嫌でないなら別のアプリケーションアオブジェクトを作成し、グローバルアプリケーションオブジェクトの代わりにそれを使うことができる。オブジェクト指向アプローチはDefault Applicationのセクションで大いに説明されている。一つの選択肢として心に留めておこう。
...
各ワイルドカードはURLの該当部分をキーワード引数としてリクエストのコールバックへ投げる。

### HTTP REQUEST METHODS
HTTPプロトコルは異なるタスク用の複数のリクエストメソッドを定義している。GETは
POST, PUT, DELETE, PATCHのような他のリクエストメソッドを扱うためにmethodキーワード引数をrouteデコレータに追加するか、あるいは代わりとなる次の五つのデコレータのうち対応するものを使いましょう。get(), post(), put(), patch()

POSTメソッドは一般的にHTMLのフォーム送信のために使用される。次の例はPOSTを使ったログインの処理方法だ。

### Special Methods: HEAD and ANY

HEADメソッドはあるGETリクエストに対応するリクエストと同一だがbodyを持たないレスポンスを要求するのに使用される。これは全てのドキュメントをダウンロードする必要性をなくして、リソースのメタ情報を得るのに役に立つ。

### ROUTING STATIC FILES
画像やCSSファイルなどの静的ファイルは自動的には配信されません。どのファイルを提供するか、そしてそれらをどこで見つけるかを制御するために、ルートとコールバックを追加する必要があります。

static_file()関数は安全で便利にファイルを配信するためのヘルパーです（Static Filesを参照）。
